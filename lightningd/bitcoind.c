...
/* `submitpackage`
 *
 * Submit a package of transactions (parent + child) to the Bitcoin backend.
 * This is used for broadcasting eltoo transactions with ephemeral anchors,
 * since the parent transaction may have a zero-value output that must be
			     ...
			     void *cb_arg)
{
	...
	log_debug(bitcoind->log, "submitpackage with %zu txs", tal_count(hextxs));
