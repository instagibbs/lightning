					       ...
					       const u8 *msg)
{
	...
	const bool option_commit_zero_fees = channel_has(channel, OPT_COMMIT_ZERO_FEES);
	...
	tx = htlc_tx(NULL, chainparams, &out, info->wscript, out_sats, htlc_wscript, fee,
		     0, option_anchor_outputs, option_anchors_zero_fee_htlc_tx, option_commit_zero_fees);
					       ...
					       const u8 *msg)
{
	 ...
	 */
	tx = htlc_tx(NULL, chainparams, &out, info->wscript, out_sats, htlc_wscript, fee,
		     cltv_expiry, option_anchor_outputs, option_anchors_zero_fee_htlc_tx, option_commit_zero_fees);
	tal_free(htlc_wscript);
	if (!tx) {
		/* Can only happen if fee > out_sats */
		channel_internal_error(channel, "Invalid onchaind_spend_htlc_timeout %s",
