...
#define OP_CHECKSIGFROMSTACKVERIFY  0xcd
...
void compute_taptree_merkle_root(struct sha256 *hash_out, u8 **scripts, size_t num_scripts)
{
    ...
    if (num_scripts == 1) {
        ...
        ok = wally_bip340_tagged_hash(tag_hash_buf, p - tag_hash_buf, "TapLeaf", hash_out->u.u8, 32);
        assert(ok == WALLY_OK);
...
void compute_taptree_merkle_root_with_hint(struct sha256 *update_merkle_root, const u8 *update_tapscript, const u8 *invalidated_opreturn_hint)
{
    ...
    ok = wally_bip340_tagged_hash(tap_hashes, sizeof(tap_hashes), "TapBranch", update_merkle_root->u.u8, 32);
    assert(ok == WALLY_OK);
}
...
u8 *make_eltoo_update_script(const tal_t *ctx, u32 update_num)
{
     ...
     * where EXPR_UPDATE(n) =
     *
     * OP_TEMPLATEHASH OP_INTERNALKEY OP_CHECKSIGFROMSTACKVERIFY <TL(n)> OP_CHECKLOCKTIMEVERIFY if n > 0
     *
     * Stack after OP_TEMPLATEHASH: [template_hash]
     * Stack after OP_INTERNALKEY: [template_hash, internal_key]
     * OP_CHECKSIGFROMSTACKVERIFY pops: sig (from witness), msg (template_hash), pk (internal_key)
     */
	u8 *script = tal_arr(ctx, u8, 0);
	add_op(&script, OP_TEMPLATEHASH);
	add_op(&script, OP_INTERNALKEY);
	add_op(&script, OP_CHECKSIGFROMSTACKVERIFY);
