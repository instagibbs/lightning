...
static struct bitcoin_tx *funding_tx_eltoo(const tal_t *ctx,
				     const struct utxo *utxo,
				     ...
				     const struct pubkey *remote_fundingkey)
{
    ...
    const struct pubkey *pubkeys[2];
    pubkeys[0] = local_fundingkey;
    pubkeys[1] = remote_fundingkey;

    /* Currently untweaked by a taptree */
    bipmusig_finalize_keys(&agg_pk,
           &keyagg_cache,
           pubkeys,
           sizeof(pubkeys),
...
int main(int argc, char *argv[])
{
	...
	weight += 1 + 1;
	/* Single output: Satoshis, script length, p2tr. */
	weight += 4 * (8 + 1 + BITCOIN_SCRIPTPUBKEY_P2TR_LEN);
