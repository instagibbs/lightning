...
/* Stub for unilateral_feerate from chaintopology.c */
...
/* Stub for create_ephemeral_anchor_cpfp from lightningd/ephemeral_anchor.c */
struct bitcoin_tx *create_ephemeral_anchor_cpfp(
	const tal_t *ctx UNNEEDED,
	struct lightningd *ld UNNEEDED,
	const struct bitcoin_tx *parent_tx UNNEEDED,
	const struct bitcoin_outpoint *anchor_outpoint UNNEEDED,
	size_t parent_weight UNNEEDED,
	u32 target_feerate UNNEEDED,
	const struct pubkey *final_key UNNEEDED,
	u64 final_key_idx UNNEEDED)
{ fprintf(stderr, "create_ephemeral_anchor_cpfp called!\n"); abort(); }

/* Stub for broadcast_package_ from chaintopology.c */
void broadcast_package_(const tal_t *ctx UNNEEDED,
			struct chain_topology *topo UNNEEDED,
			struct channel *chan UNNEEDED,
			const struct bitcoin_tx **txs UNNEEDED,
			size_t num_txs UNNEEDED,
			const char *cmd_id UNNEEDED,
			void (*cb)(struct channel *channel UNNEEDED,
				   bool success UNNEEDED,
				   const char *err UNNEEDED,
				   void *cbarg UNNEEDED) UNNEEDED,
			void *cbarg UNNEEDED)
{ fprintf(stderr, "broadcast_package_ called!\n"); abort(); }
